...
export default function PhotoCard({ photo, isFavourite, onToggleFavourite }) {
  // Use a optimized width/height request from Picsum to avoid loading giant full-res images,
  // making the grid performant and fast.
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex flex-col">
      ...
      <div className="p-4 flex items-center justify-between mt-auto">
        <span className="text-slate-700 font-semibold text-sm truncate mr-4" title={photo.author}>
          {photo.author}
        </span>
        <button
        ...
        >
          {isFavourite ? (
            <svg
              ...
              viewBox="0 0 24 24"
              fill="currentColor"
              className="w-6 h-6 text-red-500 transition-transform active:scale-90"
            >
            ...
            <svg
              ...
              strokeWidth="1.5"
              stroke="currentColor"
              className="w-6 h-6 text-slate-400 hover:text-red-500 transition-colors active:scale-90"
            >
              <path
                strokeLinecap="round"
